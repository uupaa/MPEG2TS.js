...
var MPEG2TSParser = WebModule["MPEG2TSParser"];
...
var MPEG2TS = {
    "VERBOSE":                      VERBOSE,
    "parse":                        MPEG2TSParser["parse"],                 // MPEG2TS.parse(source:Uint8Array, cursor:UINT32 = 0):MPEG2TSPacketObject
    "convertTSPacketToByteStream":  MPEG2TS_convertTSPacketToByteStream,    // MPEG2TS.convertTSPacketToByteStream(packet:MPEG2TSPacketUint8ArrayArray):ByteStreamUint8Array
  //"convertTSPacketToPESPacket":   MPEG2TS_convertTSPacketToPESPacket,     // MPEG2TS.convertTSPacketToPESPacket(packet:MPEG2TSPacketUint8ArrayArray):MPEG2TSPESPacketObjectArray
  //"convertPESPacketToByteStream": MPEG2TS_convertPESPacketToByteStream,   // MPEG2TS.convertPESPacketToByteStream(packet:MPEG2PESPacketObjectArray):ByteStreamUint8Array
};

// --- implements ------------------------------------------
function MPEG2TS_convertTSPacketToByteStream(packet) { // @arg MPEG2TSPacketUint8ArrayArray - [PAT, PMT, VIDEO_TS_PACKET, ...]
  ...
  //        var videoNALUnitObjectArray = MPEG4ByteStream.convertTSPacketToByteStream( videoByteStream );
  //    });

    var pesPacket  = MPEG2TS_convertTSPacketToPESPacket(packet);
    var byteStream = MPEG2TS_convertPESPacketToByteStream(pesPacket);

    return byteStream;
}

function MPEG2TS_convertTSPacketToPESPacket(packet) { // @arg MPEG2TSPacketUint8ArrayArray - [PAT, PMT, VIDEO_TS_PACKET, ...]
                                                      // @ret MPEG2TSPESPacketObjectArray - [{ PTS, PTS_TIME, DTS, DTS_TIME, PAYLOAD }, ...]
    ...
    for (var i = 0, iz = packet.length; i < iz; ++i) {
        ...
        var buffer = new Uint8Array( _getBufferLength(packet[i]) );
        var cursor = 0;
        ...
        // PESPacket = { PTS, PTS_TIME, DTS, DTS_TIME, PAYLOAD, ... }
    ...
    function _getBufferLength(packet) {
...
function _parseMPEG2PESPacket(subview) { // @arg Object - subview, MPEG2-TS Audio/Video byte stream.
                                         // @ret Object - MPEG2 PES Packet object. { PTS, PTS_TIME, DTS, DTS_TIME, PAYLOAD, ... }
...
//}@dev
    ...
    // TODO: PES_packet_length = 0 のケースの扱いがよくわからないので、調べる必要がある
    //
...
//}@dev
        ...
        case 0x2: // has PTS
            PTS      = _readTimeStamp(subview).time; // +5byte
            PTS_TIME = _toTimeStampNumber(PTS, "PTS");
            break;
        case 0x3: // has PTS and DTS
            PTS      = _readTimeStamp(subview).time; // +5byte
            PTS_TIME = _toTimeStampNumber(PTS, "PTS");
            DTS      = _readTimeStamp(subview).time; // +5byte
            DTS_TIME = _toTimeStampNumber(DTS, "DTS");
            break;
        }
    ...
    return {
        "PTS":      PTS,
        "PTS_TIME": PTS_TIME, // Number
        "DTS":      DTS,
        "DTS_TIME": DTS_TIME, // Number
...
function _readTimeStamp(subview) { // @arg Object - { source, cursor }
...
function _toTimeStampNumber(timeStamp, // @arg UINT - time stamp.
                            type) {    // @arg String = "" - PCR/SCR/STC -> 27MHz, MPEG1/PTS/DTS -> 90kHz
                                       // @ret Number - sec.ms
                                       // @desc MPEG2-TS time stamp to date time.
    var clock = 90 * 1000; // 90kHz
    switch (type || "") {
    case "PCR":
    case "SCR":
    case "STC":
...
// ------------------------------------------------------------------------
function MPEG2TS_convertPESPacketToByteStream(packet) { // @arg MPEG2PESPacketObjectArray
                                                        // @ret ByteStreamUint8Array
...
//{@dev
    if (VERIFY) {
        $valid($type(packet, "Array"), MPEG2TS_convertPESPacketToByteStream, "packet");
    }
//}@dev

    var byteStream = new Uint8Array( _getBufferLength(packet) );
    var cursor = 0;

    for (var i = 0, iz = packet.length; i < iz; ++i) {
        byteStream.set(packet[i]["PAYLOAD"], cursor);
        cursor += packet[i]["PAYLOAD"].length;
    }
    return byteStream;
    ...
    function _getBufferLength(packet) {
        var totalBufferLength = 0;
        for (var i = 0, iz = packet.length; i < iz; ++i) {
            totalBufferLength += packet[i]["PAYLOAD"].length;
        }
        return totalBufferLength;
    }
}
