...
function _readTSPacket(subview,   // @arg Object - subview for read ts packet. length = 188. { source, cursor }
    ...
    if (hasAdaptationField) {
        ...
        PCR  = adaptation_field["PCR"];
        OPCR = adaptation_field["OPCR"];
    }
    ...
    if (hasPayload) { // packet has payload
        ...
        case "VIDEO":
            if (MPEG2TSParser["VERBOSE"]) {
                console.info("VIDEO PACKET. PID = " + PID,
                             "PCR = "  + PCR  + "(" + (PCR  / MPEG2_BASE_CLOCK) + " sec)",
                             "OPCR = " + OPCR + "(" + (OPCR / MPEG2_BASE_CLOCK) + " sec)" );
            }
        ...
        case "AUDIO":
            if (MPEG2TSParser["VERBOSE"]) {
                console.info("AUDIO PACKET. PID = " + PID,
                             "PCR = "  + PCR  + "(" + (PCR  / MPEG2_BASE_CLOCK) + " sec)",
                             "OPCR = " + OPCR + "(" + (OPCR / MPEG2_BASE_CLOCK) + " sec)" );
            }
...
//}@dev
        ...
        if (stream_type in STREAM_TYPE) {
            ...
            if (MPEG2TSParser["VERBOSE"]) {
                console.warn("UNKNOWN StreamType: " + stream_type);
            }
        }
